...
def TFModule(C,MinK,MaxK):
	...
	if MinK != MaxK:
		...
		for kk in range(MinK,MaxK+1):
			K = kk
			Err, X, L = RunModule(X0,L0,mu1,C,I,K)
		...
		K = MinK
		Err, X, L = RunModule(C,I,K)

def WriteXL(X,L,Name):
	...
	g.write('TF\t'+'\t'.join(["M"+str(c+1) for c in range(K)])+'\n')
	for i in range(len(TF)):
		g.write(TF[i])
		for j in range(K):
			g.write('\t'+str(X[i][j]))
	...
	g.write('CellType\t'+'\t'.join(["M"+str(c+1) for c in range(K)])+'\n')
	for c in range(I):
		g.write(ct[c])
		for j in range(K):
			g.write('\t'+str(LL[c][j]))
...
if __name__ == '__main__':
	name = "CL";rna = "./Input/CL_scRNA.txt";rna_meta = "./Input/CL_scRNA_Cluster.txt";minK=4;maxK=20
